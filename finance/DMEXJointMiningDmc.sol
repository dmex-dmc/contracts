...
import "./DMEXJointMiningDmcStorage.sol";
import "./Vistor.sol";
import "./SafeMath.sol";
import "./Address.sol";
...
contract DMEXJointMiningDmc is DMEXJointMiningDmcStorage,Vistor {
    ...
    event UpdateLimitDays(uint256 limitDays);
    ...
    function _userRedeemCommon(IMasterMining masterMining, bytes32 _pid, uint256 _amount) internal {
        ...
        uint256 remaining = _amount.sub(plateformAmount);
        TransferHelper.safeTransfer(_vendorPools[_pid].global.ptoken, fundAddr, plateformAmount);
        TransferHelper.safeTransfer(_vendorPools[_pid].global.ptoken, msg.sender, remaining);
        emit UserRedeemMPool(msg.sender, _pid, remaining, _amount, _dliTokens);
    }
    ...
    function getUserIncome(bytes32 _pid) public view returns(uint256) {
        UserPool storage _userPool = _userPools[_pid];
        return _userPool.globalMask.sub(_userPool.userMask[msg.sender]).mul(_userPool.userHolders[msg.sender]).div(GLOBAL_DECIMAL)
        .add(_userPool.userRecvs[msg.sender]);
    }
